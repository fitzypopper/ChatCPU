...
class ChatCPU:
    ...
    def step(self):
        ...
        if op == 0x00:
            ...
            self.CF = int(result > 0xFFFF)
            self.A = result & 0xFFFF

        elif op == 0x06:
            result = self.A - self.B
            self.CF = int(result < 0)
            self.A = result & 0xFFFF
...
"""
    ...
    if cmd == "version":
        return await read_file("/minios/version.txt")
    ...
    if cmd == "mem":
        ...
        data = cpu.ram[address:address + length]
...
print("Commands: help ls cat disk version run asm regs mem reset clear")
